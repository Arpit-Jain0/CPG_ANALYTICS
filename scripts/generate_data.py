...
CAMPAIGN_ROWS = [
    ...
    ("CAMP004", "Personal Care", "MIDWEST",   "DIGITAL", date(2023,  8,  1), date(2023, 10, 31), 3_800_000),
    ("CAMP005", "Beverages",     "WEST",      "TV",      date(2024,  1,  1), date(2024,  3, 31), 6_200_000),
    ("CAMP006", "Frozen Foods",  "NORTHEAST", "SOCIAL",  date(2024,  4,  1), date(2024,  6, 30), 1_800_000),
]
    ...
    for dt in date_range:
        for store_id in POS_STORES:
            ...
            for sku, category, _, _, list_price, _ in PRODUCT_ROWS:
                lam = CATEGORY_BASE_RATE[category] * demand_mult(dt, category, region, promo_set, rng)
    ...
    for dt in date_range:
        for loc_id in ONLINE_LOCS:
            ...
            for sku, category, _, _, list_price, _ in PRODUCT_ROWS:
                ...
                lam = CATEGORY_BASE_RATE[category] * 0.45 * demand_mult(dt, category, region, promo_set, rng)
                for _ in range(int(rng.poisson(lam))):
...
def _make_late_rows(
    ...
    for idx in rows.index:
        ...
        if isinstance(orig, datetime):
            rows.at[idx, ts_col] = datetime(late_dt.year, late_dt.month, late_dt.day,
                                             orig.hour, orig.minute)
...
def gen_incr_pos_batch(
    ...
    # Late-arriving rows
    if n_late > 0:
        late = _make_late_rows(df, "ts", "transaction_id", _next_txn,
                               n_late, late_before, rng)
        extras.append(late)
...
def gen_incr_online_batch(
    ...
    if n_late > 0:
        late = _make_late_rows(df, "order_datetime", "order_id", _next_ord,
                               n_late, late_before, rng)
        extras.append(late)
        dq["LATE_ARRIVING"] = len(late)
...
def print_summary(stats: list[dict], seed: int) -> None:
    ...
    for s in stats:
        ...
        if dq:
            ...
            print(f"    DQ issues injected  (total dirty rows: {dirty:,} / {dirty/total_rows*100:.1f}%)")
...
def main() -> None:
    ...
    df_store = pd.DataFrame(
        STORE_ROWS, columns=["store_id", "region", "city", "store_type"]
    )
    df_calendar = make_seasonal_calendar()

    ref_path = hist / "historical_data.xlsx"
    write_multisheet(ref_path, {
    ...
    stats.append({
        ...
        "dq":      dq_pos,
        "special": ["Schema A: transaction_id / ts / store_id / sku / qty / unit_price / amount / currency"],
    })
